...
async def test_hybrid_search_with_tags_and_heat(monkeypatch, db_conn):
    ...
    await db_conn.execute(
        "INSERT INTO embeddings (content_id, vector) VALUES (?, ?)", ("ItemA", str([0.9] * 768))
    )
    await db_conn.execute(
        "INSERT INTO embeddings (content_id, vector) VALUES (?, ?)", ("ItemB", str([0.1] * 768))
    )
    await db_conn.execute(
        "INSERT INTO embeddings (content_id, vector) VALUES (?, ?)", ("ItemC", str([0.1] * 768))
    )
