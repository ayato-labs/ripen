...
async def test_save_memory_core_full_success(fake_llm):
    ...
    async with await async_get_connection() as conn:
        # Entities
        async with conn.execute("SELECT name, description FROM entities WHERE name='UnitNode'") as cursor:
            row = await cursor.fetchone()
            assert row is not None
            assert row[1] == "Unit test node"
        
        # Observations
        async with conn.execute("SELECT content FROM observations WHERE entity_name='UnitNode'") as cursor:
            row = await cursor.fetchone()
            assert row is not None
...
async def test_save_memory_core_with_conflict(fake_llm):
    ...
    await logic.save_memory_core(
        entities=[{"name": "ConflictNode", "description": "Existing"}]
    )
...
async def test_save_memory_core_ai_error_handling(fake_llm):
    ...
    with patch("shared_memory.core.logic.compute_embeddings_bulk", side_effect=Exception("AI Quota Exceeded")):
