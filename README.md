...
*   **Headers**:
    ...
    *   `X-Ripen-Agent-Id` (Optional): String identifying the agent source (e.g., `dify-translator`).
*   **Request Body**:
    ```json
    {
      "text": "The billing microservice has been migrated from port 8080 to 9090."
    }
    ```
*   **Response**: `200 OK`
    ```json
...
*   **Example (curl)**:
    ```bash
    curl -X POST http://localhost:8377/api/v1alpha2/memories \
      -H "Content-Type: application/json" \
      -H "X-Ripen-Agent-Id: my-dify-workflow" \
      -d '{"text": "Always run python tests with pytest."}'
    ```

### 2. Search Memories
Retrieves relevant memories using hybrid vector + lexical search.

*   **Endpoint**: `GET /api/v1alpha2/memories`
*   **Query Parameters**:
...
*   **Response**: `200 OK`
    ```json
    {
      "results": [
        {
          "content": "Always run python tests with pytest.",
          "score": 0.892,
          "agent_id": "my-dify-workflow",
...
*   **Example (curl)**:
    ```bash
    curl "http://localhost:8377/api/v1alpha2/memories?query=python+tests&limit=3"
    ```
...
## Scaling to Production (Enterprise Edition) 🚀

By default, Ripen-free is optimized for local/small-team workflows with a zero-config SQLite database. However, if your team scales and requires high-concurrency writes or production-grade scaling, you can upgrade to the **Enterprise Edition**.

The Ripen database access layer is fully decoupled via a dynamic registry (`RepositoryRegistry`). The Enterprise Edition replaces the SQLite backend with **PostgreSQL (pgvector + tsvector)** to support high write-concurrency, dedicated database servers, and advanced auditing.

If you are interested in upgrading to PostgreSQL support or require custom integration assistance:
*   **Contact**: [cwblog69@gmail.com](mailto:cwblog69@gmail.com)
*   For details on the trial flow and commercial terms, see [COMMERCIAL.md](COMMERCIAL.md).
