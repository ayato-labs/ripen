...
async def test_comprehensive_database_integrity(mock_llm):
    ...
    entities = [
        {
            "name": "Project X",
            "entity_type": "Project",
            "description": "A top-secret research project.",
        },
    ...
    relations = [
        {
            ...
            "object": "Project X",
            "predicate": "leads",
            "justification": "Alice was appointed as lead.",
        }
    ]
    observations = [
    ...
    async with aiosqlite.connect(db_path) as conn:
        ...
        cursor = await conn.execute(
            "SELECT * FROM relations WHERE subject = 'Alice' AND object = 'Project X'"
        )
        row = await cursor.fetchone()
...
async def test_adversarial_large_data_compression(mock_llm):
    ...
    entities = [{"name": "Mega Project", "entity_type": "LargeScale", "description": large_desc}]
...
async def test_adversarial_duplicate_conflict(mock_llm):
    ...
    await save_memory_core(
        observations=[{"entity_name": "ConflictTarget", "content": "The sky is blue."}]
    )

    # 2. Mock LLMにコンフリクトレスポンスを設定
    # システムテストでは mock_llm (MagicMock) が自動適用されるため、それを使用する
    mock_llm.models.set_response(
        "generate_content", '[{"conflict": true, "reason": "Contradicts previous knowledge."}]'
    )

    # 3. 矛盾する情報を保存
    result = await save_memory_core(
        observations=[{"entity_name": "ConflictTarget", "content": "The sky is green."}]
    )
    ...
    try:
        with pytest.raises((asyncio.TimeoutError, Exception)):
            await asyncio.wait_for(
                save_memory_core(entities=[{"name": "LockedEntity"}]), timeout=2.0
            )
    finally:
        conn_lock.rollback()
        conn_lock.close()
