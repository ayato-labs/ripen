...
2. **Private Repository (`ripen-enterprise`)**
   ...
   - **主要コンポーネント**:
     - `licensing.py`: Keygen.sh と連携したライセンス検証エンジン（署名検証、デバイス認証）。
     - `plugin.py`: Core版への機能インジェクションを担当するエントリーポイント。
     - `features/`: 商用版限定の拡張機能（Analytics等）。
   - **ライセンス**: 商用（非公開）。
...
商用ロジックの配布と、ライセンス失効時の「実質的な回収」を実現するため、以下のフローで運用します。

### 📦 配信メカニズム
- **限定配布**: `ripen-enterprise` は一般公開の PyPI には登録せず、GitHub Private Repository またはプライベート PyPI (Cloudsmith等) で管理します。
- **アクセス権の付与**: 購入者の GitHub ID または専用トークンに対し、リポジトリへの読み取り権限を動的に付与します。
- **インストール**: ユーザーは専用の `pip install` または `uv add` コマンド（認証情報込み）を使用してパッケージを導入します。

### 🛡️ ライセンスの強制力（Enforcement）
物理的にユーザーのローカル環境からコードを削除することは不可能なため、**「機能の無効化」**をもって回収に代えます。

- **起動時バリデーション**:
  `ripen-core` の起動時に、商用プラグインの `initialize()` メソッドが Keygen.sh と通信し、ライセンスの有効性を検証します。
- **機能の凍結（Feature Freezing）**:
  ライセンスが失効、またはサーバーに接続できずキャッシュも切れている場合、プラグインは商用機能（クラウド同期、SSO等）をアンロードし、コア版を OSS モードで動作させます。
- **自動アップデートの遮断**:
  ライセンス失効と同時にリポジトリへのアクセス権を剥奪することで、ユーザーは最新のセキュリティパッチや新機能を受け取ることができなくなります。

### ⚖️ 法的・規約的制約
- **EULA (エンドユーザーライセンス契約)**:
  「ライセンス失効後は商用パッケージを速やかに削除すること」を明記し、契約上の返却義務を課します。
